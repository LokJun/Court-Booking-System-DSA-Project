...
struct User
{
    ...
    string name;
    string phoneNumber;
    int loginAttempts = 0; // Add login attempts counter
    time_t lockTime = 0; // Add time of lock
};
...
public:
    ...
    // Function to check if user is locked and unlock if 5 minutes have passed
    bool isUserLocked(const string& username, vector<User>& users) {
        for (auto& user : users) {
            if (user.username == username) {
                ...
                if (currentTime > user.lockTime) {
                    user.lockTime = 0; // Unlock the user
        ...
        return false; // User not found (shouldn't happen in normal flow)
    }

    // Function to lock a user
    void lockUser(const string& username, vector<User>& users) {
        for (auto& user : users) {
            if (user.username == username) {
                user.lockTime = time(0) + 5 * 60; // Lock for 5 minutes
                user.loginAttempts = 0; // Reset login attempts
                break;
            }
        }
    ...
    // Function to save the updated user data to file
    void saveUsers(const vector<User>& users, const string& filename) {
        ofstream file(filename, ios::trunc); // Open in truncate mode to overwrite
        if (file.is_open()) {
            for (const auto& user : users) {
                file << user.username << " " << user.password << " " << user.name << " " << user.phoneNumber << " " << user.loginAttempts << " " << user.lockTime << endl;
            }
            file.close();
        } else {
    ...
    // Modify the loginUser function to handle locking
    bool loginUser(vector<User>& usersList, string& loggedInName, string& loggedInPhone, string& loggedInUsername) {
        if (usersList.empty()) {
        ...
        if (isUserLocked(usernameInput, usersList)) {
        ...
        while (true) {
            ...
            for (auto& user : usersList) {
                if (user.username == usernameInput && user.password == passwordInput) {
                    ...
                    saveUsers(usersList, "users.txt"); // Save the updated user list
                    return true;
                }
            }
            ...
            for (auto& user : usersList) {
                if (user.username == usernameInput) {
                    ...
                    if (user.loginAttempts >= 3) {
                        lockUser(usernameInput, usersList);
            ...
            saveUsers(usersList, "users.txt"); // Save the updated user list
            if (attempts >= 3) {
                break; // Exit the loop after 3 attempts even if not locked
            }
        }
...
void editCourt(vector<Court>& courts, const string& filename)
{
    ...
    if (it != courts.end())
    {
        ...
        cout << "Enter new Available Slots (e.g., 08:00 - 22:00) or press Enter to keep current: ";
        string newSlots;
...
void initializeFiles()
{
    ...
    if (!fileExists("courts.txt"))
    {
        ...
        for (int i = 1; i <= 20; ++i)
        {
            courtsFile << i << " " << "BI" << i << " " << 20.00 << " Available Badminton 08:00-22:00 Indoor" << endl;
        }
                for (int i = 21; i <= 25; ++i)
        {
            courtsFile << i << " " << "BO" << i << " " << 10.00 << " Available Badminton 08:00-19:00 Outdoor" << endl;
        }
        for (int i = 26; i <= 45; ++i)
        {
            courtsFile << i << " " << "PO" << (i-25) << " " << 20.00 << " Available Pickleball 07:00-19:00 Outdoor" << endl;
        }
                for (int i = 46; i <= 50; ++i)
        {
            courtsFile << i << " " << "PI" << i << " " << 25.00 << " Available Pickleball 08:00-22:00 Indoor" << endl;
        }
        courtsFile.close();
    }
...
// Modify the LoadUsers function to load the new fields
vector<User> LoadUsers(const string& filenameUsers)
{
    ...
    User user;
    // Read data for each user from the file
    while (file >> user.username >> user.password >> user.name >> user.phoneNumber) 
    {
        // Handle potential missing loginAttempts and lockTime for older files
        if (!(file >> user.loginAttempts)) {
            user.loginAttempts = 0; // Default value if not found
            file.clear(); // Clear error state
            file.ignore(numeric_limits<streamsize>::max(), '\n'); //skip the current line
        } else if (!(file >> user.lockTime)) {
            user.lockTime = 0; // Default value if not found
            file.clear(); // Clear error state
            file.ignore(numeric_limits<streamsize>::max(), '\n'); //skip the current line
        }
...
void customerInput(vector<int> courtIDs, const string& name, const string& contact, const string& username)
{
    ...
    // Ã¥Â»ÂºÃ§Â«â€¹ Receipt Ã§â€°Â©Ã¤Â»Â¶
    ...
    cout << "==================================" << endl;
    // Ã¥â€šÂ³Ã©â‚¬Â²Ã¤Â¾â€ Ã§Å¡â€ž name Ã¨Ë†â€¡ contact Ã¥Â·Â²Ã§Â¶â€œÃ¤Âºâ€¹Ã¥â€¦Ë†Ã¨Â¼Â¸Ã¥â€¦Â¥Ã¯Â¼Å’Ã§â€žÂ¡Ã©Å“â‚¬Ã¥â€ ÂÃ¦Â¬Â¡Ã¨Â¼Â¸Ã¥â€¦Â¥
    ...
    for (int courtID : courtIDs) {
        ...
        // Ã¨Â¼Â¸Ã¥â€¦Â¥Ã¦â€”Â¥Ã¦Å“Å¸Ã¯Â¼Ë†Ã¥Â¤Â©/Ã¦Å“Ë†/Ã¥Â¹Â´Ã¯Â¼â€°
        do
        {
            ...
            // Ã©Â©â€”Ã¨Â­â€°Ã¦ËœÂ¯Ã¥ÂÂ¦Ã§â€šÂºÃ¦Å“ÂªÃ¤Â¾â€ Ã§Å¡â€žÃ¦Å“â€°Ã¦â€¢Ë†Ã¦â€”Â¥Ã¦Å“Å¸
        ...
        // Ã¨Â¼Â¸Ã¥â€¦Â¥Ã© ÂÃ¨Â¨â€šÃ§Å¡â€žÃ¦â„¢â€šÃ¦Â®Âµ
        while (true) {
        ...
        // Ã¨Â¼Â¸Ã¥â€¦Â¥Ã© ÂÃ¨Â¨â€šÃ¥Â°ÂÃ¦â„¢â€š
        while (true) {
        ...
        // Ã¦ÂªÂ¢Ã¦Å¸Â¥Ã¦â€°â‚¬Ã©ÂÂ¸Ã§Å¡â€žÃ§ÂÆ’Ã¥ Â´Ã¥Å“Â¨Ã¨Â¼Â¸Ã¥â€¦Â¥Ã§Å¡â€žÃ¦â€”Â¥Ã¦Å“Å¸Ã¨Ë†â€¡Ã¦â„¢â€šÃ¦Â®ÂµÃ¦ËœÂ¯Ã¥ÂÂ¦Ã¥ÂÂ¯Ã§â€Â¨
        ...
        // Ã©Â¡Â¯Ã§Â¤ÂºÃ¨Â¼Â¸Ã¥â€¦Â¥Ã§Å¡â€žÃ¨Â¨â€šÃ¤Â½ÂÃ¨Â³â€¡Ã¨Â¨Å 
    ...
    // Ã¨Â®â‚¬Ã¥Ââ€“Ã¦â€°â‚¬Ã¦Å“â€°Ã§ÂÆ’Ã¥ Â´Ã¨Â³â€¡Ã¦â€“â„¢
    vector<Court> allCourts = readCourtsFromFile("courts.txt");

    // Ã¥Ââ€“Ã¥Â¾â€”Ã§â€ºÂ®Ã¥â€°ÂÃ¥Â·Â²Ã¥Â­ËœÃ¥Å“Â¨Ã§Å¡â€žÃ¦Å“â‚¬Ã¥Â¤Â§ booking ID
    int bookingId = 1;
    ...
    // Ã¥Â°â€¡Ã¨Â¨â€šÃ¤Â½ÂÃ¨Â³â€¡Ã¨Â¨Å Ã¥Â¯Â«Ã¥â€¦Â¥ "booked_courts.txt"
    ofstream bookedFileOut("booked_courts.txt", ios::app);
    if (bookedFileOut.is_open()) {
        for (Booking& booking : bookings) {
            // Ã¦â€°Â¾Ã¥Ë†Â°Ã¥Â°ÂÃ¦â€¡â€°Ã§Å¡â€žÃ§ÂÆ’Ã¥ Â´Ã¨Â³â€¡Ã¨Â¨Å 
...
void generateAdminReport(const vector<Court>& courts, vector<Booking>& bookings) {
    ...
    auto displayReport = [&](const vector<Booking>& filteredBookings) {
        ...
        cout << "\nBookings by Court Type & Preference:\n";
...
void bookingMenu(const string& username, const string& name, const string& phoneNumber) {
    ...
    // Ask user to choose the number of courts to book using dynamic memory allocation
    int* numberOfCourtsPtr = new int; // Allocate memory for an integer on the heap
    ...
    while (true) {
        ...
        if (isBackCommand(numInput)) {
            ...
            delete numberOfCourtsPtr; // Release the allocated memory before returning
            return;
        }

        try {
            *numberOfCourtsPtr = stoi(numInput); // Store input in the dynamically allocated int
            if (*numberOfCourtsPtr <= 0) {
                throw invalid_argument("Non-positive number");
            }
            if (*numberOfCourtsPtr > courts.size()) {
                cout << "\nSorry, we don't have enough courts.\n";
                continue;
            }
            break;
        }
    ...
    for (int i = 0; i < *numberOfCourtsPtr; ++i) { // Use the value pointed to by numberOfCourtsPtr
        string courtNumber;
        while (true) {
            ...
            if (isBackCommand(courtNumber)) {
                ...
                delete numberOfCourtsPtr; // Release memory before returning
                return;
            }
    ...
    delete numberOfCourtsPtr; // Release the dynamically allocated memory
}
